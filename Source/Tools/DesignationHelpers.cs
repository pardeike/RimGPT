...
namespace RimGPT
{
    public static class DesignationHelpers
    {
        public static (string order, string targetLabel) GetOrderAndTargetLabel(Designation des)
        {
            ...
            string translationKey = des.def.defName == "HarvestPlant"
            ...
            string stuffLabel = des.target.Thing?.Stuff?.label ?? "";
            string baseTargetLabel = des.target.HasThing
            ...
            string targetLabel = string.IsNullOrEmpty(stuffLabel) ? baseTargetLabel : $"{stuffLabel} {baseTargetLabel}";
