...
#pragma warning restore S1104 // Fields should not have public accessibility
        ...
        public bool IsConfigured =>
            (IsApiKeySet || Tools.LocalAIEnabled) && ((azureSpeechKey?.Length > 0 && azureSpeechRegion?.Length > 0) || showAsText);

        private bool IsApiKeySet =>
            chatGPTKey?.Length > 0 || openRouterKey?.Length > 0;
            ...
            if (IsApiKeySet)
            {
                ...
                if (Widgets.ButtonText(rect, ApiProviderPrimary))
                    UX.ApiProviderMenu(l =>
                    {
                        ...
                        //Logger.Message($"currentConfig: {JsonConvert.SerializeObject(OpenAIApi.currentConfig, Configuration.JsonSerializerSettings)}"); // TEMP
                    });
